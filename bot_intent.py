...
- approve_candidate(index)     确认处理某条候选，index 从当前候选列表推断
                               用户会用自然语言描述内容（包括标题、有趣点、账号名、关键词），
                               对应候选列表中的序号
                               触发词不限于"发"，"看""选""要""这个""那个"都可以触发
                               示例："发那个讲 claude 的"、"发 sama 的那条"、"发第一个"、"要那条关于 AI 的"
                                     "我要看这个趣闻"、"我要看那个关于鸡块的"、"我选第一条"
                                     "这个有意思，发出去"、"就那个 AI 写论文的"、"第二条更好"
...
关键词搜索：
- viral(keyword)               关键词爆文搜索，找该领域最有价值的单条推文翻译发布
                               示例："搜一下 AI agents 的爆文"、"找找最近 xx 的热门推文"
                               "xx 领域有什么高热度的"、"帮我找一条关于 xx 的好推文"
- digest(keyword)              话题综述，搜索 X + 网络新闻，生成中文综述文章发到 Telegram
                               示例："给我写个 AI agents 的综述"、"xx 最近有什么动态"
                               "帮我整理一下 xx 领域"、"xx 领域最近怎么样"
- discover_fun(n?)             开放式趣文发现，用 xAI 搜索本周最有趣的推文并进入候选队列
                               n 默认 1，最多 3
                               示例："找一条有趣的推文"、"发现趣文"、"找找好玩的"
                               "有没有什么有趣的推文"、"来一条有意思的"、"找个好笑的"
...
对于涉及实际操作的 action（viral、digest、deliver、score_and_present、scrape），
根据用户语气判断是否需要二次确认：

- **needs_confirm: true**：用户是询问/描述语气，不是明确指令
  例：「AI agents 最近怎么样」「sama 发了什么好东西」「有没有值得发的内容」
  此时 reply 改为确认提示：「帮你搜一下「AI agents」的爆文？」

- **needs_confirm: false**：用户是明确指令语气，直接执行
  例：「搜一下 AI agents 的爆文」「给我写个综述」「帮我发一条」「发1」

纯查询类操作（status、list_accounts、list_scores 等）无论语气都不需要确认。
...
- keyword: string，搜索关键词（viral / digest 用）
